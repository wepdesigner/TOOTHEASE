...
`;
fs.writeFileSync(path.join(__dirname, '../src/Pages/Admin/AdminMemberships.jsx'), componentCode);

// 2. Modify AdminPanel.jsx
const adminPanelPath = path.join(__dirname, '../src/Pages/Admin/AdminPanel.jsx');
let adminPanelCode = fs.readFileSync(adminPanelPath, 'utf8');

if (!adminPanelCode.includes('AdminMemberships')) {
  // Add import
  adminPanelCode = adminPanelCode.replace(
    'import "./Styles/Admin.css";',
    'import "./Styles/Admin.css";\nimport AdminMemberships from "./AdminMemberships";'
  );
  
  // Add to NAV under FINANCE & COMMS
  adminPanelCode = adminPanelCode.replace(
    '{ id:"payments",     icon:"Y\'", label:"Payments" },',
    '{ id:"payments",     icon:"Y\'", label:"Payments" },\n      { id:"memberships",  icon:"ti-chart-bar", label:"SaaS MRR" },'
  );
  
  // Add to render view
  adminPanelCode = adminPanelCode.replace(
    '{view === "settings" && <div className="page-anim"><p>Settings Coming Soon.</p></div>}',
    '{view === "settings" && <div className="page-anim"><p>Settings Coming Soon.</p></div>}\n            {view === "memberships" && <AdminMemberships />}'
  );
  
  fs.writeFileSync(adminPanelPath, adminPanelCode);
  console.log("Updated AdminPanel.jsx to include SaaS MRR tab.");
}
