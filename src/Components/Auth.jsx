...
export default function Auth() {
  ...
  const handleLogout = useCallback(() => {
    // 1) Clear persisted UI session
    LS.del(SESSION_KEY);

    // 2) Clear API/JWT token used by axios interceptor
    localStorage.removeItem("token");

    // 3) Clear any other likely auth-related keys
    try {
      localStorage.removeItem("stech_session");
      localStorage.removeItem("token");
    } catch {}

    // 4) Hard redirect to login route to avoid any panel sticking
    // (Some doctor routes are rendered via <Routes> independently.)
    setUser(null);
    setView("login");
...
function DashboardRouter({ user, onLogout, showToast }) {
  switch (user.role) {
    case "admin":
      return <ApiAdminPanel admin={user} onLogout={onLogout} />;

    case "doctor":
      return <ApiDoctorPanel doctorId={user.id} doctor={user} onLogout={onLogout} showToast={showToast} />;
...
function PatientDashboardWrapper({ patient: sessionUser, onLogout, showToast }) {
  const [patient, setPatient] = useState(sessionUser);
